...
#define DECKTHREADCOUNT	4


#define UIHACKTIME		500
#define UIALARMTIME		1500
...
public:
	...
	virtual bool			GetSteamInitialized();
#endif

	idStr					GetAutocomplete(  idStrList candidates, idStr partialString );
	void					KillDeckThreads();
	bool					RunDeckCommand( const char *rawCommand );
	bool					inCutscene;
	void					CallMap( const char *funcname );
	const char *			ParseTime(float value);
	const char *			ParseTimeMS(int value);
	void					ParseStickynotes(const char *text, bool local);
	idStr					ParseStickyLine(idStrList lines, idStr key);
	void					KillCCTVs( void );

	idStr					GetMapIndex( void );
	bool					GetMapInterstitial( void );

	
	idUserInterface			*mainGui;		//main menu gui.

#ifdef STEAM
	//bc public
	bool					steamInitialized;
	void					ShowWorkshopLegalGui( void );
	bool					workshopLegalOpen;
	idUserInterface			*workshopLegalGui;	//main menu gui.
	bool					textinputOn;
	...
	idList<SteamLeaderboard_t>	steamLeaderboardHandles;
	idList<idStr>				steamLeaderboardNames;
	idListGUI *					leaderboardMaplist;		// in the mainmenu leaderboard, this is the map list.

	idListGUI *					mainmenuLeaderboardNames;
	idListGUI *					mainmenuLeaderboardTimes;
...
private:
	...
	bool				steamGotStickynotes;
	

	idStrList			leaderboardList;
	bool				hasInitializedLeaderboardlist;

	PublishedFileId_t	modFileIDs[MAX_WORKSHOP_ITEMS];
	
	STEAM_CALLBACK( idGameLocal, OnTextDismissed, GamepadTextInputDismissed_t, m_SteamCallResultTextDismissed );
#endif

	
	int lastbugreportcount;

	int nextAutocompleteTime;

	void				SpawnEntFile(); //BC 4-16-2026
...
#include "keypad.h"
#include "lever.h"
#include "contractcamera.h"
#include "clock.h"
#include "tablefold.h"
#include "camerapoint.h"
#include "jellypoint.h"
#include "doorfolding.h"
#include "worldman_moveable.h"
#include "launcher.h"
#include "countdowntimer.h"
#include "commentary.h"
#include "button_switcher.h"
#include "cybervendor.h"
#include "qglass.h"
#include "trembler.h"
#include "bluebox.h"
#include "animloop.h"
