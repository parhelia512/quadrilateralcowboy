...
static void Cmd_Hudmessage_f( const idCmdArgs &args )
{
	idStr text;

	if ( args.Argc() < 2 )
	{
		gameLocal.Printf( "usage: hudmessage <text>\n" );
		return;
	}
...
void Cmd_deckexit_f( const idCmdArgs &args )
{
...
void idGameLocal::InitConsoleCommands( void ) {
	cmdSystem->AddCommand( "hudmessage",			Cmd_Hudmessage_f,			CMD_FL_GAME,				"display message on hud" );
...
//	cmdSystem->AddCommand( "clientCallVote",		idMultiplayerGame::CallVote_f,	CMD_FL_GAME,			"call a vote: clientCallVote si_.. proposed_value" );
	...
	cmdSystem->AddCommand( "deckexit",				Cmd_deckexit_f,				CMD_FL_GAME,				"exit deck." );
